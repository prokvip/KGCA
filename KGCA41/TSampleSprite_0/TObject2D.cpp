...
void  TObject2D::UpdateVertexBuffer()
{
    ...
    m_VertexList[5].p = { m_vNDCPos.x + m_vDrawSize.x, m_vNDCPos.y - m_vDrawSize.y, 0.0f };
...
void  TObject2D::SetPosition(TVector2D vPos)
{
    m_vPos = vPos;
    // 0  ~ 800   -> 0~1 ->  -1 ~ +1
    m_vNDCPos.x = (vPos.x / g_rtClient.right) * 2.0f - 1.0f;
    m_vNDCPos.y = -((vPos.y / g_rtClient.bottom) * 2.0f - 1.0f);
