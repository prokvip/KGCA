...
#pragma region  temp
			if (name == L"upperarm_twist_01_l")//, upperarm_l
			{
				name = L"upperarm_l";
			}
#pragma endregion
...
{
	if (m_pActionModel != nullptr)
	{
		m_fCurrentAnimTime += m_pActionModel->GetFrameSpeed() * g_fSecondPerFrame * 1.0f;
		if (m_fCurrentAnimTime >= m_pActionModel->GetEndFrame() - 1)
		{
			float fTime = m_fCurrentAnimTime - m_pActionModel->GetEndFrame();
			m_fCurrentAnimTime = m_pActionModel->GetStartFrame();
			//m_fCurrentAnimTime += fTime;
		}
	}
	//  0   b0 ~ b4
	//  1   b0 ~ b4
	...
	if (m_pActionModel != nullptr)
	{
		...
		{	
			TFbxObj* pSrc = m_pModel->m_TreeList[inode].get();

#pragma region temp
			if (pSrc->m_szName == L"upperarm_twist_01_l")//, upperarm_l
			{
				pSrc->m_szName = L"upperarm_l";
			}
#pragma region
			//// name <- inode = 10 --> m_pModel
			////UINT iActionIndex = m_pActionModel.find(name) 해당하는 인덱스를 찾는다.;
			TFbxObj* pDest = nullptr;
			for (int iActionIndex =0; iActionIndex < m_pActionModel->m_TreeList.size(); iActionIndex++)
			{
				pDest = m_pActionModel->m_TreeList[iActionIndex].get();
				if (pSrc->m_szName == pDest->m_szName)
				{
					m_matBoneArray.matBoneWorld[inode] =
						m_pActionModel->m_MatrixArray[m_fCurrentAnimTime][iActionIndex];
					break;
				}				
			}			
		}
