...
bool TWorldObject::Load(ID3D11Device* pd3dDevice,
	...
	T_STR name)
{
	...
	if (pCharacter)
	{
		...
		m_pFbxFile = m_pCharacter->GetFbxFile();
		m_AnimScene = m_pCharacter->m_AnimScene;
		m_ActionCurrent = m_pCharacter->m_ActionCurrent;
		m_fAnimFrame = m_pCharacter->m_fAnimFrame;
		m_iStartFrame = m_pCharacter->m_iStartFrame;
		m_iEndFrame = m_pCharacter->m_iEndFrame;
		m_fAnimInverse = m_pCharacter->m_fAnimInverse;
		m_fAnimSpeed = m_pCharacter->m_fAnimSpeed;
	}
	CreateConstantBuffer();
	return true;
}
HRESULT	TWorldObject::CreateConstantBuffer()
{
	HRESULT hr;
...
bool TWorldObject::UpdateFrame(ID3D11DeviceContext* pContext)
{
	m_fAnimFrame = m_fAnimFrame +g_fSecondPerFrame * m_fAnimSpeed *	m_AnimScene.fFrameSpeed;// *m_fAnimInverse;
	if (m_fAnimFrame > m_ActionCurrent.iEndFrame ||	m_fAnimFrame < m_ActionCurrent.iStartFrame)
	{
...
void	TWorldObject::SetMatrix(TMatrix* matWorld, TMatrix* matView, TMatrix* matProj)
{
	if (matWorld != nullptr)
	{
		m_matWorld = *matWorld;
...
bool TWorldObject::PreRender(ID3D11DeviceContext* pContext )
{
	return true;
}
bool TWorldObject::Render(ID3D11DeviceContext* pContext )
{
...
bool TWorldObject::RenderShadow(ID3D11DeviceContext* pContext)
{
	pContext->VSSetConstantBuffers(1, 1, &m_pAnimBoneCB);
	for (int iMesh = 0; iMesh < m_pFbxFile->m_pDrawObjList.size(); iMesh++)
	{
...
bool TWorldObject::PostRender(ID3D11DeviceContext* pContext)
{
	return true;
}
bool TWorldObject::Release()
{
