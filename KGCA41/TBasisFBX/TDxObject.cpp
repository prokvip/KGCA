...
namespace TBasisFBX
{
	...
	{
		m_pColorTex = I_TextureFBX.Load(szColorFileName);
		if (szMaskFileName != nullptr)
		{
			m_pMaskTex = I_TextureFBX.Load(szMaskFileName);
		}
	...
	bool    TDxObject::SetConstantData()
	{
		...
		m_ConstantList.matWorld = TBasisFBX::TMatrix();
		m_ConstantList.matView = TBasisFBX::TMatrix();
		m_ConstantList.matProj = TBasisFBX::TMatrix();
		m_ConstantList.matNormal = TBasisFBX::TMatrix();
	...
	{
		m_pVShader = I_ShaderFBX.CreateVertexShader(m_pd3dDevice,
			szFile, "VS");
		return true;
	}
	bool    TDxObject::CreatePixelShader(const TCHAR* szFile)
	{
		m_pPShader = I_ShaderFBX.CreatePixelShader(m_pd3dDevice,
			szFile, "PS");
		return true;
	}
	...
	bool	TDxObject::Frame(float fSecperFrame, float fGameTimer)
	{
		return true;
	}
	bool    TDxObject::Draw()
	{
		if (m_pContext == nullptr) return true;
