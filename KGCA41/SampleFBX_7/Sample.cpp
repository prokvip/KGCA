...
bool Sample::Init()
{
	...
	// fbx 
	TFbxObj* pFbxObj[6];	
	pFbxObj[0] = TModelMgr::Get().Load(L"../../res/fbx/SK_Mannequin.FBX", L"CharacterWeight.hlsl");
	pFbxObj[1] = TModelMgr::Get().Load(L"../../res/fbx/ThirdPersonIdle.FBX", L"CharacterWeight.hlsl");
	//pFbxObj[2] = TModelMgr::Get().Load(L"../../res/fbx/ThirdPersonWalk.FBX", szShaderFile);
	//pFbxObj[3] = TModelMgr::Get().Load(L"../../res/fbx/ThirdPersonRun.FBX", szShaderFile);
	...
	auto obj1 = std::make_shared<TMapObjSkinning>();
	obj1->m_pModel = pFbxObj[0];
	obj1->m_pActionModel = pFbxObj[1];
	obj1->CreateBoneBuffer();
	m_MapObj.push_back(obj1);
	...
	m_pDebugCamera->CreatePerspectiveFov(T_PI * 0.25, (float)
		...
		1.0f, 30000.0f);
	m_pDebugCamera->CreateLookAt({ 0,0,-500 }, { 0,0,0 });
