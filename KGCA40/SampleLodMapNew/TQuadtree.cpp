...
DWORD TQuadtree::GetIndexCounter(DWORD dwData, DWORD dwNumCell)
{
...
bool TQuadtree::ComputeStaticLodIndex(int iMaxCells)
{
	...
	for (DWORD dwLevel = 0; dwLevel < (DWORD)m_iNumPatch; dwLevel++)
	{
		...
		TLodPatch* pStaticData = &m_LodPatchList[dwLevel];
		pStaticData->iLodLevel = dwLevel + 1;
		for (int iData = 0; iData < 16; iData++)
		{
			...
			CreateIndexBuffer(m_LodPatchList[dwLevel], iData);
		}	
		//for (int iCode = 0; iCode < 16; iCode++)
		//{
		//	CreateIndexBuffer(m_LodPatchList[dwLevel], iCode);
		//}
	}
	return true;
}